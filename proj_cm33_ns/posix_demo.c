...
#define THREAD_EXIT_CONDITION                  (3)
#define SLEEP_MICROSEC                         (200000)
#define DELAY_MS                               (10)
...
void* queue_receive_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        if ((exit_status & THREAD_EXIT_CONDITION) == THREAD_EXIT_CONDITION )
        {
            pthread_exit(NULL);
        }
...
void* posix_demo(void *arg)
{
    ...
    /* This task was created with xTaskCreate() API function, so
